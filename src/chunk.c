...
void init_chunk(chunk_t *chunk, i32 cx, i32 cz, struct osn_context *ctx)
{
    ...
    for (i32 y = 0; y < CHUNK_HEIGHT; y++)
    {
        for (i32 z = 0; z < CHUNK_DEPTH; z++)
        {
            for (i32 x = 0; x < CHUNK_WIDTH; x++)
            {
                f32 xx = (f32)(x + cx);
                f32 zz = (f32)(z + cz);
                f32 n = fabs(open_simplex_noise2(ctx, (float)(x + cx * CHUNK_WIDTH + ((float)rand_r(&SEED) / (RAND_MAX))) / 80, (float)(z + cz * CHUNK_DEPTH + ((float)rand_r(&SEED) / (RAND_MAX))) / 80) * 12);
                // f32 n = 5 * sin(x);
    ...
    for (i32 x = 0; x < CHUNK_WIDTH; x++)
    {
        for (i32 y = 0; y < CHUNK_HEIGHT; y++)
        {
            for (i32 z = 0; z < CHUNK_DEPTH; z++)
            {
                if (get_type(get_block(chunk, x, y, z)) != GRASS)
                    continue;
...
{
    if (y > CHUNK_HEIGHT - 5 || x < 5 || x > 11 || z < 5 || z > 11)
        return;
